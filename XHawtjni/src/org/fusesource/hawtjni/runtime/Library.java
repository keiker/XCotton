...
public class Library {
    ...
    public Library(String name, String version) {
        this(name, null, version, null);
    }
    
    public Library(String name, Class<?> clazz) {
        this(name, null, version(clazz), clazz.getClassLoader());
    }
    
    public Library(String name, String extension, Class<?> clazz) {
        this(name, extension, version(clazz), clazz.getClassLoader());
    }
    
    public Library(String name, String extension, String version, ClassLoader classLoader) {
        if( name == null ) {
    ...
    final public String getArchSpecifcResourcePath() {
        return "native/"+ getPlatform() + "/" + System.getProperty("os.arch") + "/" +map(name);
    }
    ...
    final public String getPlatformSpecifcResourcePath(String platform) {
        return "native/"+platform+"/"+map(name);
    }

    final public String getResorucePath() {
        return "native/"+map(name);
    }
    ...
    private boolean exractAndLoad(ArrayList<String> errors, String version, String customPath, String resourcePath) {
        ...
        if(res != null) {
            ...
            if( customPath!=null ) {
                ...
                File target = res instanceof URL ? extract(errors, resource, prefix, suffix, file(customPath)) : extract(errors, is, prefix, suffix, file(customPath));
                if( target!=null ) {
    ...
    private String map(String libName) {
        ...
        if(extension != null && !"".equals(extension)){
        	libName = libName.substring(0, libName.lastIndexOf(".") + 1) + extension;
        }
    ...
    private File extract(ArrayList<String> errors, InputStream is, String prefix, String suffix, File directory) {
        File target = null;
        if (directory != null) {
          directory = directory.getAbsoluteFile();
        }
